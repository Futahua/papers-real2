// A2.0C probe harness — clean CODEX_HOME App Server isolation. No thread, no model turn.
'use strict';
...
const WORKSPACE = 'D:\\LapSlop brotherhood\\Programs\\Papers are papers\\_agent_gate1_probe_fresh_20260711\\temporary-repositories\\a2-clean-home-fixture';
const CLEAN_HOME = 'D:\\LapSlop brotherhood\\Programs\\Papers are papers\\_agent_gate1_probe_fresh_20260711\\temporary-codex-home\\a2-clean-home';
const EVENTS_OUT = 'D:\\LapSlop brotherhood\\Programs\\Papers are papers\\_agent_gate1_probe_fresh_20260711\\evidence\\gate-a\\a2-clean-home-events.jsonl';
const LAUNCH_ARGS = ['app-server', '--listen', 'stdio://'];
const COMMAND_ARGV = ['C:\\Program Files\\PowerShell\\7\\pwsh.exe', '-NoProfile', '-Command', 'Write-Output A2_CLEAN_HOME_OK'];
...
(async () => {
  let execResult = null, execError = null, reportedCodexHome = null, cleanHomeApplied = false;
  try {
    const init = await request('initialize', { clientInfo: { name: 'papers-a2-cleanhome-probe', title: 'Papers A2.0C Clean Home Probe', version: '0.0.1' } });
    reportedCodexHome = init.codexHome || null;
    cleanHomeApplied = reportedCodexHome === CLEAN_HOME;
    notify('initialized', {});
    if (!cleanHomeApplied) {
      record('probe', { reportedCodexHome, expected: CLEAN_HOME }, 'CLEAN HOME NOT APPLIED — stopping without command attempt');
    } else {
