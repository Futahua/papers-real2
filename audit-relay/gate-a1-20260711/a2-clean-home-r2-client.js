// A2.0C-R2 probe harness — clean CODEX_HOME isolation with normalized path comparison. No thread, no model turn.
'use strict';
...
const WORKSPACE = 'D:\\LapSlop brotherhood\\Programs\\Papers are papers\\_agent_gate1_probe_fresh_20260711\\temporary-repositories\\a2-clean-home-r2-fixture';
const CLEAN_HOME = 'D:\\LapSlop brotherhood\\Programs\\Papers are papers\\_agent_gate1_probe_fresh_20260711\\temporary-codex-home\\a2-clean-home-r2';
const EVENTS_OUT = 'D:\\LapSlop brotherhood\\Programs\\Papers are papers\\_agent_gate1_probe_fresh_20260711\\evidence\\gate-a\\a2-clean-home-r2-events.jsonl';
const LAUNCH_ARGS = ['app-server', '--listen', 'stdio://'];
const COMMAND_ARGV = ['C:\\Program Files\\PowerShell\\7\\pwsh.exe', '-NoProfile', '-Command', 'Write-Output A2_CLEAN_HOME_R2_OK'];

// Mandated normalization: strip \\?\ prefix, win32 normalize, trim trailing separators (except drive root), case-insensitive.
...
const env = Object.assign({}, process.env, { CODEX_HOME: CLEAN_HOME });
const child = spawn(CODEX_EXE, LAUNCH_ARGS, { stdio: ['pipe', 'pipe', 'pipe'], cwd: WORKSPACE, env });
...
function respond(id, result) {
  ...
  record('client->server', msg, 'conservative denial response');
  child.stdin.write(JSON.stringify(msg) + '\n');
}

const state = { approvalRequestObserved: false, errors: [] };
...
(async () => {
  let execResult = null, execError = null, reportedCodexHome = null;
  let normReq = normalizeWinPath(CLEAN_HOME), normRep = null, cleanHomeApplied = false, commandAttempted = false;
  try {
    const init = await request('initialize', { clientInfo: { name: 'papers-a2-cleanhome-r2-probe', title: 'Papers A2.0C-R2 Clean Home Probe', version: '0.0.1' } });
    reportedCodexHome = init.codexHome || null;
    normRep = normalizeWinPath(reportedCodexHome);
    cleanHomeApplied = normReq !== null && normReq === normRep;
    record('probe', { requested: CLEAN_HOME, reported: reportedCodexHome, normalizedRequested: normReq, normalizedReported: normRep, cleanHomeApplied }, 'codexHome comparison');
    notify('initialized', {});
    if (!cleanHomeApplied) {
      record('probe', {}, 'NORMALIZED PATHS DIFFER — stopping without command attempt');
    } else {
  ...
  let cleanHomeTopLevelNames = [];
  try { cleanHomeTopLevelNames = fs.readdirSync(CLEAN_HOME); } catch (_) {}
  ...
  console.log(JSON.stringify({
    ...
    exit: exited,
    reportedCodexHome,
    normalizedRequested: normReq,
    normalizedReported: normRep,
    cleanHomeApplied,
    commandAttempted,
    execResult,
    execError,
    ...
    unknownEvents,
    errors: state.errors,
    stderr: sanitize({ s: stderrBuf }).s,
    cleanHomeTopLevelNames,
