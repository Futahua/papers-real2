...
class PapersPatchController {
  ...
  deny(id){const p=this._claim(id,'denied');this.approvals.submitDecision(p.approvalId,'deny');this._replace(p,{papersStatus:'denied',providerStatus:'decline-sent'});return {resolution:'denied'};}
  async apply(id) {
    ...
    try {
      ...
      const receipt=createReceipt({proposalId:p.proposalId,provider:p.provider,model:p.model,threadId:p.threadId,turnId:p.turnId,itemId:p.itemId,approvalRequestId:p.approvalRequestId,providerDecision:'decline',providerTerminalStatus:terminal,repositoryRoot:'selected-disposable-worktree',branch:p.capture.branch,headBefore:p.capture.head,affectedPaths:p.affectedPaths,preApplyHashes:p.capture.affectedFileHashes,...result,startedAt,completedAt:new Date().toISOString(),outcome:'applied-by-papers',failureCode:null});
